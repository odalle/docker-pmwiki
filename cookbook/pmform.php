...
$RecipeInfo['PmForm']['Version'] = '20140212';
...
if (function_exists('Markup_e'))
  Markup_e('pmform', '<input',
    '/\\(:pmform *([-\\w]+)( .*?)?:\\)/',
    "PmFormMarkup(\$pagename, \$m[1], \$m[2])");
else
  Markup('pmform', '<input',
    '/\\(:pmform *([-\\w]+)( .*?)?:\\)/e',
...
function PmFormSave($pagename, $msgtmpl, $opt, $safe_opt) {
  global $IsPagePosted, $ChangeSummary, $Now;
  Lock(2);
